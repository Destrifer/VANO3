# -*- coding: utf-8 -*-
# Разбор консолидированного ядра (формат владельца) -> читаемый отчёт + csv.
# Использование: python _core_report.py <tag> <xlsx_path>
import zipfile, re, os, csv, sys
...
tag=sys.argv[1]; path=sys.argv[2]
d=parse(path)
...
def add(q, cluster, demand, comp='', group=''):
    ...
    if not q: return
    r=queries.get(q)
    if not r: queries[q]={'query':q,'cluster':cluster or '','demand':demand,'comp':comp or '','group':group or ''}
    else:
...
for name,rows in d.items():
    ...
    hdr=rows[0]
    qi=col_with(hdr,'апрос'); di=col_with(hdr,'прос (')
    if di<0: di=col_with(hdr,'прос')
    ci=col_with(hdr,'онкурент'); cli=col_with(hdr,'ластер'); gi=col_with(hdr,'руппа')
    if qi<0 or di<0: continue
    for r in rows[1:]:
        if len(r)<=max(qi,di): continue
        add(r[qi] if qi<len(r) else '', r[cli] if (0<=cli<len(r)) else '',
            num(r[di]) if di<len(r) else 0, r[ci] if (0<=ci<len(r)) else '', r[gi] if (0<=gi<len(r)) else '')

qs=list(queries.values())

NONTARGET=['сайт ','лендинг','верстк','бесплатно','конструктор','генератор','редактор','нейросет','нейро ',
 'продвижени','раскрутк',' seo','сео ','скачать','читать онлайн','читать журнал','смотреть онлайн','программа для',
 'приложени','фотошоп','photoshop','word','ворд','corel','canva','андроид','айфон','iphone','значение','означает',
 'сонник','приснил','к чему снится','epub','fb2','pdf скач','подписк','подписат']
INFO=['как сделать','как создать','как нарисовать','как оформить','как распечат','как печатать','как сверстать',
 'что такое','чем отличается','своими руками','в домашних','в ворде','шаблон','образец','пример оформлен','идеи']
...
lines=[f"ВСЕГО уникальных запросов: {len(qs)}  суммарный спрос: {sum(q['demand'] for q in qs)}","По категориям (n / спрос):"]
cnt=Counter(q['cat'] for q in qs); dem=defaultdict(int)
...
for cat in ['PRINT','INFO','BRANDED','NONTARGET']:
    a=agg(cat); lines.append(f"================= {cat}  [кластеров={len(a)}] =================")
    for n,c in a:
        top=' | '.join(f"{x['query']}({x['demand']})" for x in c['qs'][:10])
        lines.append(f"CLUSTER: {n}  [n={c['n']} спрос={c['demand']}]"); lines.append(f"   {top}")
    lines.append("")
open(os.path.join(HERE,f'report_{tag}.txt'),'w',encoding='utf-8').write('\n'.join(lines))
os.makedirs(os.path.join(HERE,'data'),exist_ok=True)
with open(os.path.join(HERE,'data',f'queries_{tag}_master.csv'),'w',encoding='utf-8-sig',newline='') as f:
