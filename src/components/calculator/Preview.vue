...
function drawFolded(ctx: CanvasRenderingContext2D, cssW: number, cssH: number) {
  ...
  // Геометрия панелей по типу фальцовки. Точка зрения — СБОКУ СВЕРХУ, поэтому
  // панель это не параллелограмм, а трапеция: удалённый край поднят (`rise`) и
  // короче (`far` < `near`). Параллелограмм с равными высотами читался как
  // взгляд СНИЗУ — крылья задирались вверх «домиком».
  // roll («Евро», «Улитка») — крайние панели завёрнуты ВНУТРЬ, средние стоят
  // фронтально. accordion («Гармошка») и book («Книжка») — зигзаг: ближний и
  // дальний край чередуются от панели к панели.
  const raw: { c: Pt[]; shade: number }[] = [];
  const rolled = foldKind.value === "roll";
  // У рулонной угол должен читаться сильнее: там крайние реально загнуты внутрь,
  // а гармошка лишь качается вокруг фронтальной плоскости.
  const rise = rolled ? depth * 2.2 : depth * 1.5;
  const near = ph;
  const far = ph * (rolled ? 0.8 : 0.85); // перспективное укорочение вдаль
  // Разворот всей сцены: без него рулонная выходит симметричной, а симметрия
  // читается как взгляд строго в лоб (и «домиком»), сколько трапеции ни строй.
  // Сдвигаем каждую следующую панель вверх — объект встаёт вполоборота.
  const tilt = rolled ? rise * 0.42 : 0;
  let lx = 0;
  for (let i = 0; i < panels; i++) {
    // yT* — верхняя кромка края, h* — его высота
    let yTL: number, yTR: number, hL: number, hR: number, shade: number, wI: number;
    if (rolled) {
      if (i === 0) {
        wI = pw * 0.62;
        yTL = -rise; hL = far; yTR = 0; hR = near; shade = -0.22;
      } else if (i === panels - 1) {
        wI = pw * 0.62;
        yTL = 0; hL = near; yTR = -rise; hR = far; shade = -0.28;
      } else {
        // средние фронтальные; лёгкая ступенька, чтобы у «Улитки» (4 панели)
        // они не слились в одну плиту
        wI = pw;
        const k = (i - 1) / Math.max(1, panels - 3);
        yTL = -k * rise * 0.12; hL = near;
        yTR = -(k + 0.5) * rise * 0.12; hR = near;
        shade = 0.06 - (i - 1) * 0.08;
      }
    } else {
      wI = pw;
      const leftNear = i % 2 === 0; // сгибы чередуются → ближний край тоже
      yTL = leftNear ? 0 : -rise; hL = leftNear ? near : far;
      yTR = leftNear ? -rise : 0; hR = leftNear ? far : near;
      shade = leftNear ? 0.06 : -0.18;
    }
    const rx = lx + wI;
    const tL = -tilt * i;
    const tR = -tilt * (i + 1);
