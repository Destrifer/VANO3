...
export class SvgContext {
  ...
  private useHalftone(ink: number) {
    ...
    // Только средние тона. Нижняя граница важнее верхней: на очень светлой
    // заливке точки расходятся так далеко, что читаются не как тон, а как сор —
    // на этом «карман» папки (12% краски) превратился в точечный клин и выглядел
    // как ошибка рендера. Верхняя — чтобы плотный тон не слипался в плашку.
