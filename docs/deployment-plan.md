...
- ✅ **Шаг 5.1** — master = деплой-ветка (FF-влили infra/deployment → master; триггер push на master + `paths-ignore` для `**.md`/`docs`/`seo`).
- ✅ **Шаг 6** — авто-пересборка по публикации контента. Directus Flow «Deploy site on publish» (event hook на products/promoted_pages → операция request POST в `https://api.github.com/repos/Destrifer/VANO3/dispatches`, body `{"event_type":"deploy-site"}`) → workflow `repository_dispatch: types:[deploy-site]`. Проверено end-to-end: правка продукта → пересборка → деплой. **Грабли:** (1) repository_dispatch стартует только из дефолтной ветки → workflow должен быть на master; (2) заголовок должен быть `Authorization: Bearer <PAT>` — без `Bearer ` GitHub отдаёт 401 «Requires authentication»; (3) PAT classic со scope `repo`. `concurrency: cancel-in-progress: true` схлопывает частые правки; (4) **грабля №6 (починено 2026-07-01, `3e26b9b`):** контентные пересборки были пустышками — Docker кешировал слой `RUN npm run build` (cache-from gha) при неизменном коде и НЕ перечитывал Directus; CI «зелёный», а сайт старый. Фикс: `ARG CACHEBUST` перед build + build-arg `CACHEBUST=${{github.run_id}}-${{github.run_attempt}}`. Симптом на будущее: правишь контент, деплой зелёный, на сайте старое → `curl -sI` Last-Modified; (5) ~~Flow ловил только `products`/`promoted_pages`~~ — ✅ 2026-07-02: «Deploy site on publish» расширен на `products_gallery` + `items.delete`; новый Flow «Deploy site on file edit» (`directus_files`, только update) — правка подписи файла деплоит, массовая загрузка файлов — нет.
- ✅ **Шаг 7** — CWV измерены (Lighthouse mobile, lab): главная 100, `/business-cards` 99; LCP/CLS/TBT(INP) в зелёной зоне. Полевые CrUX появятся после запуска и трафика (Search Console).
- ✅ **Шаг 12 (бэкапы)** — на сервере `/opt/printmos/backup.sh` (симлинк `pmos-backup`) дампит БД (`pg_dump --clean | gzip`) + аплоады (tar.gz) в `/opt/printmos/backups/{db,uploads}`, ротация (30 БД / 14 аплоадов). **Cron ежедневно 03:30.** Откат — `pmos-restore` (интерактивно: список → выбор; перед откатом делает страховочный `pre-restore` бэкап, останавливает app, restore, поднимает). Перед рискованной правкой прод-БД: `pmos-backup before-edit`. Скрипты в репо `ops/`. ⚠️ Бэкапы на том же сервере (защита от плохих правок, НЕ от потери сервера) — для off-site периодически качать на локалку (`scp -i ~/.ssh/printmos_deploy root@72.56.245.83:/opt/printmos/backups/db/<файл> .`).
- ✅ **Directus обновлён 11.17.4 → 12.1.1 (2026-07-02):** OIG-ключ в `.env.production` (`DIRECTUS_LICENSE_KEY` → `LICENSE_KEY`), `IP_TRUST_PROXY=true` (12 сменил default на false, мы за Caddy). Бэкап отката — `before-directus12`. **Грабля №7:** в образе 12 нет `npx` — CLI = `node /directus/cli.js` (шаг schema apply в CI поправлен); 12 требует абсолютный `PUBLIC_URL`.
- ✅ **Схема как код — автоматизировано (2026-07-02):** шаг «Apply Directus schema» в `deploy.yml` — scp `directus/snapshot.yaml` на сервер → `docker compose exec directus npx directus schema apply --yes` ДО сборки Astro (идемпотентно). Обновление снапшота с прода — `ops/schema-snapshot.sh` (дисциплина: правка схемы на проде → снапшот → коммит; устаревший снапшот откатит прод-схему).
- ⏳ Остаток: уведомления (Telegram/SMTP), off-site бэкапов, снять noindex на запуске, почистить 5 дев-заказов, хардненинг админа (локальный токен всё ещё валиден на проде!).
...
- [x] Влить `infra/deployment` → `master`, переключить триггер workflow на `master` — ✅ (шаг 5.1).
